    ...
    describe("Unparseable strings", () => {
        ["-", "", undefined, null, "Alle meine Entchen", [], 0].forEach((unparseableString) => {
            it(`the invalid string '${unparseableString}' returns a zero cost object`, () => {
                expect(parseCostString(unparseableString)).to.deep.equal(new Costs(0, 0, false));
            });
        });
    });
    describe("Actual costs", () => {
        const validCosts = {
            K2: new Costs(2, 0, true),
            K2V1: new Costs(1, 1, true),
            K2V2: new Costs(0, 2, true),
            "20V10": new Costs(10, 10, false),
            "5000V100": new Costs(4900, 100, false),
            "K1000V300": new Costs(700, 300, true),
            "1V1": new Costs(0, 1, false),
            "1": new Costs(1, 0, false),
        };
    ...
    describe("Invalid costs", () => {
        ["K2V3", "K2V-1", "K2V", "K1V0", "3V5", "V5"].forEach((invalidCost) => {
            it(`Returns a zero cost object for ${invalidCost}`, () => {
                expect(parseCostString(invalidCost)).to.deep.equal(new Costs(0, 0, false));
            });
        });
    });
});